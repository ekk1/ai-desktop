...
- [x] **Step 4: 写失败的 Schema 3→4 迁移和深拷贝测试**
...
- [x] **Step 5: 实现迁移、验证委托和 `UpdateVideos`**
...
- [x] **Step 1: 写失败的 Batch/Item 顺序和 Timing 校验测试**
...
- [x] **Step 4: 写失败的不可变快照、状态转换和重启恢复测试**
...
- [x] **Step 1: 写失败的 active 输入、归档保留和顺序引用测试**
...
- [x] **Step 1: 写失败的递归 Merge、`null` 删除和 Timing 测试**
...
- [x] **Step 4: 写失败的 Asset Data URL、顺序和快照测试**
...
- [x] **Step 5: 实现 Cancel 和共享 HTTP Helper 的最小提取**
...
- [x] **Step 1: 写失败的只暂存已选 Asset 和硬链接回退测试**
...
- [x] **Step 4: 写失败的 Shell 引用和 RAW 变量测试**
...
- [x] **Step 1: 写失败的预设共享并发和 Item 单活动测试**
...
- [x] **Step 4: 写失败的 HTTP 轮询、实际帧数和不可取消测试**
...
- [x] **Step 6: 写失败的 CLI 工作区、取消、日志和结果导入测试**
...
- [x] **Step 7: 实现 CLI 状态机、事件与 Shutdown**
...
- [x] **Step 2: 运行尾帧测试并确认 RED**

Run: `go test ./internal/videogen -run TestTail -count=1 -v`

- [x] **Step 3: 实现提取 Repository、模板和进程生命周期**
...
- [x] **Step 1: 写失败的完整配置和 Capabilities API 测试**
...
- [x] **Step 2: 写失败的 Batch/Item CRUD 和执行 API 测试**
...
- [x] **Step 3: 写失败的 CLI 日志、工作区和尾帧 API 测试**
...
- [x] **Step 5: 写失败的真实 App 组装、重启和关闭测试**
...
- [x] **Step 6: 组装 Repository/Service/Manager/Tail 并加入关闭顺序**
...
- [x] **Step 1: 写失败的嵌入资源和表单 Contract 测试**
...
- [x] **Step 2: 运行静态 Contract 并确认 RED**
...
- [x] **Step 1: 写失败的页面结构和无依赖 Contract 测试**
...
- [x] **Step 2: 运行页面 Contract 并确认 RED**

Run: `go test ./internal/web -run TestEmbeddedVideoWorkspace -count=1 -v`

- [x] **Step 3: 实现 Batch 列表、标题/文件夹和 Item 编辑**
...
- [x] **Step 4: 接入 active Asset Picker 与有序角色**
...
- [x] **Step 5: 实现运行、SSE、CLI 日志和浏览器清屏**
...
- [x] **Step 4: 运行真实 HTTP 和 CLI Fixture 验收**
...
在 1440px 与 390px 宽度检查：左栏/抽屉、标题文件夹、Timing 互斥、Item 素材顺序、结果 `<video>`、折叠高级信息、直接可见错误、CLI 浏览器清屏和 Tail 操作；持续输出 CLI/Tail 日志，确认页面字节窗口不会超过 SSE 快照声明的容量。记录检查结果到视频规格交付记录。
